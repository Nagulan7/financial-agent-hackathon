...
def analyze_all_users_with_progress(df):
    """Analyze all users and return results with progress bar"""
    unique_user_ids = df['user_id'].unique()
    
    progress_bar = st.progress(0)
    status_text = st.empty()
    
    # Use the refactored function but add progress tracking
    results = {}
    
    for i, user_id in enumerate(unique_user_ids):
        status_text.text(f'Processing User {user_id}... ({i+1}/{len(unique_user_ids)})')
        progress_bar.progress((i + 1) / len(unique_user_ids))
        
        # This will be handled by the refactored function
        
    progress_bar.empty()
    status_text.empty()
    
    # Call the actual analysis function
    _, results = load_and_analyze_for_streamlit(df=df)
    return results
